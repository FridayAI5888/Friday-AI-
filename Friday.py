...
def needs_search(text):
    text = text.lower()
    keywords = [
        "latest","current","recent","new","today","yesterday","news","update",
        "suggest recent","trending","search for","look up","who made",
        "who is creator of","in 2026","in 2025","who is"
    ]
...
def assistant_reply(user_input):
    ...
    for n in range(1, 8):
        ...
        if f"{n}{suffix} message" in text_lower or f"what was my {n}{suffix} message" in text_lower:
            if st.session_state.conversation_log and len(st.session_state.conversation_log) >= n:
                ...
                reply = f"Your {n}{suffix} message was: \"{msg}\" 🙂"
            else:
                reply = f"You haven't sent a {n}{suffix} message yet."
...
for key in [
    f"chats_{current_user}",
    f"history_{current_user}",
    f"conversation_log_{current_user}",
    f"memory_{current_user}",
    f"current_chat_name_{current_user}",
    f"name_finalized_{current_user}"
]:
